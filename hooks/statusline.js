...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // --- Context window (generic: works with ANY model) ---
    let ctx = '';
    ...
    if (haveAbs) {
      // Known context window → show percentage bar + absolute numbers
      const used = Math.max(0, Math.min(100, Math.round((totalInput / ctxSize) * 100)));
      ...
      const abs = ` ${fmt(totalInput)}/${fmt(ctxSize)}`;
      ...
      // Absolute token cap: 250k is the pricing/quality cliff
      if (totalInput >= 250000 && color === '\x1b[38;2;255;125;218m') {
        color = '\x1b[33m';
      }

      ctx = ` ${color}${prefix}${bar}${abs}\x1b[0m`;
    } else if (totalInput > 0) {
      // Unknown context window → show raw token count with "?" indicator
      ctx = ` \x1b[33m${fmt(totalInput)} used\x1b[0m\x1b[2m(?%)\x1b[0m`;
    }
